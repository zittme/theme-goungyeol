(function () {
	...
	ready(function () {
		...
		if (ssSearchBtn && ssMainrow) {
			var ssSearchInput = ssMainrow.querySelector('.ss-search input[type="search"]');

			function closeHeadSearch() {
				ssMainrow.classList.remove('is-search-open');
				ssSearchBtn.setAttribute('aria-expanded', 'false');
			}

			ssSearchBtn.addEventListener('click', function () {
				var open = !ssMainrow.classList.contains('is-search-open');
				ssMainrow.classList.toggle('is-search-open', open);
				ssSearchBtn.setAttribute('aria-expanded', open ? 'true' : 'false');
				if (open && ssSearchInput) { ssSearchInput.focus(); }
			});

			ssMainrow.addEventListener('click', function (e) {
				if (e.target.closest('[data-ss-search-close]')) { closeHeadSearch(); }
			});

			ssMainrow.addEventListener('keydown', function (e) {
		...
		var counter = visual.querySelector('#hrVisualNow');
		var progress = visual.querySelector('.hr-visual-bar i');
